...
(function() {
    ...
    function initBalatroBackground() {
        // 檢查 OGL 是否已載入
        if (typeof window.OGL === 'undefined' && typeof Renderer === 'undefined') {
            // 如果還沒載入，等待一下再試（最多等待 5 秒）
            let attempts = 0;
            const maxAttempts = 50; // 5 秒
            const checkInterval = setInterval(() => {
                attempts++;
                if (typeof window.OGL !== 'undefined' || typeof Renderer !== 'undefined' || attempts >= maxAttempts) {
        ...
        const RendererClass = OGL.Renderer || Renderer;
        const ProgramClass = OGL.Program || Program;
        const MeshClass = OGL.Mesh || Mesh;
        const TriangleClass = OGL.Triangle || Triangle;

        if (!RendererClass) {
            console.error('BalatroBackground: Renderer class not found');
            return;
        }
