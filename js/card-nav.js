...
(function() {
    ...
    function initCardNav() {
        ...
        if (!cardNav || !hamburgerMenu || !menuWrap || !menuItems.length) {
        ...
        function calculateHeight() {
            ...
            if (isMobile) {
                // 手機版：根據視窗高度動態計算，確保項目可正常顯示
                const viewportHeight = window.innerHeight;
                const availableHeight = viewportHeight - topBar - 15; // 減少邊距，讓選單更緊湊
                const itemHeight = Math.floor(availableHeight / itemCount);
                
                // 確保每個項目有合理的最小高度（縮小間距，讓選單更緊湊）
                const minItemHeight = 50; // 從 60 減少到 50
                const maxItemHeight = 70; // 從 80 減少到 70
                const finalItemHeight = Math.max(minItemHeight, Math.min(itemHeight, maxItemHeight));
                
                // 計算總高度，減少間距讓項目更緊密
                const totalHeight = topBar + (finalItemHeight * itemCount) - 5; // 減少 5px 讓項目更緊密
                const maxAllowedHeight = viewportHeight * 0.9; // 最大不超過視窗的90%
